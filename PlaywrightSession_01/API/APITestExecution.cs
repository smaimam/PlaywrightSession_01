...
namespace PlaywrightPOM
{
    ...
    public class APITestExecution : PlaywrightTest
    {
        ...
        public async Task POST()
        {
            ...
            data.Add("id", "2");
            data.Add("title", "Automation");
            data.Add("author", "amir");
            var newcomment = await Request.PostAsync("/posts", new() { DataObject = data });
            Assert.IsTrue(newcomment.Ok);

            var getResponse = await API.GETRequest(@"http://localhost:3000/posts", "title", "Automation");
